-- FIELD-FLOW activity ingestion conflict-target fix.
--
-- The original function used column names in ON CONFLICT that also exist as
-- PL/pgSQL variables. Targeting the table's named unique constraint avoids an
-- ambiguous-column error while preserving idempotent ingestion.
